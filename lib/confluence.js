 ...
 * @param {Function} callback
 */
Confluence.prototype.putLabel = function(space, id, version, title, content, callback){
    var page = {
        id: id,
        type: "page",
        title: title,
    ...
    request
        .put(this.config.baseUrl + this.config.apiPath + "/content/" + id + this.config.extension + "?expand=body.storage,version,metadata.labels")
        .auth(this.config.username, this.config.password)
        .type('json')
        .send(page)
        .end(function(err, res){
...
/**
 *  This allows you to search in plugins for query 
 *
 * @param {string} path
 * @param {string} query
 * @param {Function} callback
 */
Confluence.prototype.pluginsSearch = function(path, query, callback){

    request
        .get(this.config.baseUrl + path + query)
        .auth(this.config.username, this.config.password)
        .end(function(err, res){
            processCallback(callback, err, res);
        });
